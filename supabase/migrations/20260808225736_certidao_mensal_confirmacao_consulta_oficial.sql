-- SafeScan Brasil
-- J.52-P7-R1B — confirmação transacional da consulta oficial
--
-- Mantém a assinatura da RPC existente.
-- Ao confirmar um documento que exige consulta oficial e está
-- com consulta PENDENTE, a mesma transação promove a consulta
-- para CONFIRMADA.
-- DIVERGENTE continua impedindo decisão CONFORME.
...
declare
    ...
    v_requer_consulta_oficial boolean;
    v_status_consulta_oficial text;
    v_status_consulta_oficial_destino text;
...
begin
    ...
    select
        ...
        i.versao_atual_id,
        i.status,
        i.requer_consulta_oficial,
        i.status_consulta_oficial
    into
        v_competencia_id,
        v_versao_atual_id,
        v_status_anterior,
        v_requer_consulta_oficial,
    ...
    v_status_consulta_oficial_destino :=
        case
            when v_decisao = 'CONFORME'
             and coalesce(
                    v_requer_consulta_oficial,
                    false
                 )
             and v_status_consulta_oficial = 'PENDENTE'
                then 'CONFIRMADA'

            else v_status_consulta_oficial
        end;
    ...
    values (
        ...
        jsonb_build_object(
            ...
            'statusConsultaOficialAnterior',
                v_status_consulta_oficial,

            'statusConsultaOficial',
                v_status_consulta_oficial_destino,

            'consultaOficialConfirmadaNestaDecisao',
                (
                    v_status_consulta_oficial = 'PENDENTE'
                    and
                    v_status_consulta_oficial_destino = 'CONFIRMADA'
                ),
...
is
    'Registra decisão humana transacional e, quando aplicável, confirma a consulta oficial do documento da Certidão Mensal.';
