-- SafeScan Brasil
-- J.52 — Validade documental histórica por competência
--
-- A validade documental de competências históricas deve usar
-- o último dia da competência como data de referência.
--
-- O diagnóstico original da versão permanece preservado.
-- Esta migration altera apenas a regra transacional utilizada
-- na decisão humana do documento.

do $preflight$
begin
    if to_regprocedure(
        'public.registrar_decisao_certidao_mensal_documento(uuid,uuid,text,text,text,timestamp with time zone)'
    ) is null then
        raise exception
            'RPC registrar_decisao_certidao_mensal_documento não localizada.';
    end if;
    ...
    if to_regclass(
        ...
        raise exception
            'Tabela certidao_mensal_versoes não localizada.';
    end if;
end;
$preflight$;
...
declare
    ...
    v_competencia date;
    v_data_referencia_competencia date;
...
begin
    ...
    v_data_validade_texto :=
        coalesce(
            nullif(
                btrim(
                    coalesce(
                        v_diagnostico_versao
                            #>> '{avaliacao,dadosTemporais,dataValidadeIso}',
                        ''
                    )
                ),
                ''
     ...
     * Regra histórica:
     *
     * Se existe uma data de validade comparável, ela é a fonte
     * de verdade para a competência.
     *
     * Somente quando a data não pode ser obtida conservamos o
     * bloqueio legado baseado nos estados já persistidos.
     */
    if v_decisao = 'CONFORME'
       and (
            (
                v_data_validade is not null
                and
                v_data_validade <
                    v_data_referencia_competencia
            )
            or
            (
                v_data_validade is null
                and (
                    v_status_anterior = 'VENCIDO'
                    or
                    v_status_resultado_versao = 'VENCIDO'
                    or upper(
                        coalesce(
                            v_diagnostico_versao
                                #>> '{avaliacao,codigo}',
    ...
    then
        raise exception using
            errcode = '22023',
            message = 'Documento vencido na data de referência da competência não pode ser confirmado como conforme.';
    end if;
...
is
'Registra decisão documental usando o último dia da competência como referência para validade histórica.';