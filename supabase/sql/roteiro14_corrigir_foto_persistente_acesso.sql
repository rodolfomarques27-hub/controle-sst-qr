-- Roteiro 14 - Pacote 9AC
-- Correção definitiva da foto em Acessos do App.
-- Objetivos:
-- 1) garantir coluna foto_url na tabela usuarios_permissoes_sistema;
-- 2) recriar RPCs de leitura/listagem retornando foto_url;
-- 3) recriar RPC de salvamento preservando foto_url existente quando nenhum novo caminho for enviado.
--
-- Importante: este SQL não apaga arquivos do Storage.
...
-- Remove todas as assinaturas antigas para evitar conflito no PostgREST.
do $$
declare
    f record;
begin
...
begin
    ...
    v_pode_gerenciar :=
        ...
        and (
            ...
            or lower(coalesce(v_admin.perfil, '')) = 'administrador'
    ...
    set
        ...
        foto_url = coalesce(v_foto_url, u.foto_url),
        perfil = v_perfil,
        ativo = v_ativo,
